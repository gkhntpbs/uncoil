            ...
            VStack(spacing: 12) {
                if hasClaude {
                    ...
                    OnboardingCard(
                        ...
                        detail: String(localized: "Hooks are Claude Code's own mechanism, so there is nothing to install here. If you add Claude Code later, come back through “Finish setup” in the sidebar — or Settings › Uncoil › Agent Status Hooks.")
                    )
                }
    ...
    var body: some View {
        OnboardingScaffold(
            ...
            VStack(spacing: 12) {
                ...
                Text("Every tool answers {\"action\":\"help\"} with its own actions, so an agent discovers what it may do. A child agent's grants are intersected with its parent's — never widened. You can change all of this later in Settings › Uncoil › Permissions.")
                    .font(Theme.ui(.small))
                    .foregroundStyle(Theme.textFaint)
                    .fixedSize(horizontal: false, vertical: true)

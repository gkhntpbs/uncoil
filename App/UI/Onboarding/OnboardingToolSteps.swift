        ...
        OnboardingScaffold(
            ...
            VStack(spacing: 12) {
                ForEach(providers) { provider in
                    OnboardingCard(
                        // A card about one agent is marked with that agent.
                        // Three cards under one generic glyph made the choice
                        // read as three copies of the same setting, which is
                        // the one thing this page must not say.
                        leading: { ProviderMark(provider: provider, size: 18) },

...
private struct RunConfigurationEditor: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            field(String(localized: "id (slug)"), text: $id, disabled: !isNew)
            field(String(localized: "Name"), text: $name)
            field(String(localized: "Command"), text: $command)
            field(String(localized: "Working directory (relative to the root)"), text: $cwd)
            field(String(localized: "Env (KEY=value, one per line)"), text: $envText, axis: .vertical)
            field(String(localized: "Ports (comma-separated)"), text: $portsText)
            field(String(localized: "Preview URL"), text: $previewURL)
            field(String(localized: "Ready pattern (regex)"), text: $readyPattern)
            field(String(localized: "Dependencies (ids, comma-separated)"), text: $dependsOnText)
